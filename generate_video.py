...
def run_open_sora_inference(params):
    image_path, prompt, file_mount_name, sample_name, duration, resolution, aspect_ratio = params
...
def main(image_paths, prompts, file_mount_names, sample_names, durations, resolutions, aspect_ratios):
    ...
    # Prepare inputs for Function.map
    inputs = [
        (
            image_paths[idx], 
            prompts[idx], 
    ...
    # Run tasks in parallel using Function.map
    results = list(run_open_sora_inference.map(inputs))
